...
export default function reviewpage({route}) {
  ...
  const {data: postdata} = useQuery(getpost, {variables: {postid: postid}});
  ...
  if (postdata) {
    let temp = 0;
    for (var i = 0; i < postdata.getpost.reviews.length; i++) {
      temp = temp + postdata.getpost.reviews[i].rating;
    }
    post_rating = temp / postdata.getpost.reviews.length;
  }
        ...
        <View>
          {postdata && (
            <Image
              ...
              source={{uri: postdata.getpost.imageurl}}
            />
          )}
          ...
          {postdata && review === true && (
            <FlatList
              data={postdata.getpost.reviews}
          ...
          {postdata && review === false && (
            <FlatList
              data={postdata.getpost.photos}
              keyExtractor={photo => photo.imgurl.toString()}
              renderItem={({item}) => (
                <Image

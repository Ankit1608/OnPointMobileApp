const {Users, Posts, Notification, Report, Feedback} = require('./models/User');
...
const resolvers = {
  ...
  Mutation: {
    addreport: async (_, {userid, postid, username}) => {
      const report = new Report({userid, postid, username});
      try {
        await report.save();
      } catch (err) {
        console.log(err);
    ...
    addfeedback: async (_, {feedback}) => {
      const res = new Feedback({feedback: feedback});
      try {
        await res.save();
      } catch (err) {
        console.log(err);
